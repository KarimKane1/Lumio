    ...
    for (const user of publicUsers) {
      try {
        ...
        if (user.email) {
          ...
          // Phone-only user - create email in the old format
          authData.email = `${user.phone_e164.replace(/[^0-9]/g, '')}@dev.jokko.local`;
          authData.email_confirm = true;
        } else {
        ...
        // Use the service role to create user with specific ID
        const { data: authUser, error: authError } = await supabase.auth.admin.createUser({
          ...authData,
          // This is the key - we need to use the same ID
          user_id: user.id
        });
        ...
        if (authError) {
          ...
          const identifier = user.email || user.phone_e164 || user.id;
          console.log(`Successfully created auth user for ${identifier} with ID ${user.id}`);
          results.push({ 
            identifier, 
            status: 'created',
